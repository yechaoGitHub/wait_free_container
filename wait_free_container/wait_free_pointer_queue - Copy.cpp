...
int64_t wait_free_pointer_queue::enqueue_range(void** arr_elem, int64_t count)
{
    ...
    if (resized)
    {
        ...
        resize(&arr_elem[fill_count], remain_count);
        m_stuck_enqueue = 0;
    }
    else
    {
...
int64_t wait_free_pointer_queue::dequeue_range(void** elem, int64_t arr_size, int64_t* index)
{
    ...
    for (int64_t i = 0; i < count; i++)
    {
        while (true)
        {
            ...
            {
                if (InterlockedCompareExchange64(reinterpret_cast<volatile long long*>(&m_data[de_pos + i]), 0, reinterpret_cast<long long>(old_value)) == reinterpret_cast<long long> (old_value))
                {
                    elem[i] = old_value;
                    break;
                }
...
int64_t wait_free_pointer_queue::resize()
{
    ...
    int64_t length = m_size * 1.5;
    void** new_data = new void* [length] { 0 };
    assert(new_data);
...
int64_t wait_free_pointer_queue::resize(void** extra_copy, int64_t size)
{
    //队列满后，出列元素，在入列元素，会导致resize重复调用，只允许第一个争抢到的线程运行resize
    int64_t old_value = mutex_check_strong(m_reszing, m_enqueuing, m_dequeuing);
    if (old_value != 0)
    ...
    int64_t length = (m_size + size) * 1.5;
    void** new_data = new void* [length] { 0 };
    assert(new_data);
    ...
    {
        en_pos = (m_enqueue_count + m_offset) % length;
